...
#define USBFSH_LOW_SPEED_DIRECT_PRE_EN    0

volatile uint8_t  USBFSH_LastPid;
volatile uint8_t  USBFSH_LastTog;
volatile uint8_t  USBFSH_LastRet;
volatile uint8_t  USBFSH_LastIntFg;
volatile uint8_t  USBFSH_LastIntSt;
volatile uint8_t  USBFSH_LastMisSt;
volatile uint8_t  USBFSH_LastHostCtrl;
volatile uint16_t USBFSH_LastHostSetup;
volatile uint16_t USBFSH_LastRxLen;
volatile uint16_t USBFSH_LastWaitLeft;

static void USBFSH_SaveTransDiag( uint8_t endp_pid, uint8_t endp_tog, uint8_t ret, uint16_t wait_left )
{
    USBFSH_LastPid = endp_pid;
    USBFSH_LastTog = endp_tog;
    USBFSH_LastRet = ret;
    USBFSH_LastIntFg = USBFSH->INT_FG;
    USBFSH_LastIntSt = USBFSH->INT_ST;
    USBFSH_LastMisSt = USBFSH->MIS_ST;
    USBFSH_LastHostCtrl = USBFSH->HOST_CTRL;
    USBFSH_LastHostSetup = USBFSH->HOST_SETUP;
    USBFSH_LastRxLen = USBFSH->RX_LEN;
    USBFSH_LastWaitLeft = wait_left;
}
...
void USBFSH_SetSelfSpeed( uint8_t speed )
{
    ...
    else
    {
        USBFSH->BASE_CTRL |= USBFS_UC_LOW_SPEED;
        USBFSH->HOST_CTRL |= USBFS_UH_LOW_SPEED;
#if USBFSH_LOW_SPEED_DIRECT_PRE_EN
        USBFSH->HOST_SETUP |= USBFS_UH_PRE_PID_EN;
#else
        USBFSH->HOST_SETUP &= ~USBFS_UH_PRE_PID_EN;
#endif
...
uint8_t USBFSH_Transact( uint8_t endp_pid, uint8_t endp_tog, uint32_t timeout )
{
    ...
    do
    {
        ...
        {
            USBFSH_SaveTransDiag( endp_pid, endp_tog, ERR_USB_UNKNOWN, i );
        ...
        else // Complete transfer 
        {
            ...
            {
                USBFSH_SaveTransDiag( endp_pid, endp_tog, ERR_SUCCESS, i );
                return ERR_SUCCESS;
            }
            ...
            if( r == USB_PID_STALL )
            {
                USBFSH_SaveTransDiag( endp_pid, endp_tog, ( r | ERR_USB_TRANSFER ), i );
        ...
        if( USBFSH->INT_FG & USBFS_UIF_DETECT )
        {
            ...
            if( USBFSH_CheckRootHubPortEnable( ) == 0x00 )
            {
                USBFSH_SaveTransDiag( endp_pid, endp_tog, ERR_USB_CONNECT, i );
    ...
    USBFSH_SaveTransDiag( endp_pid, endp_tog, ERR_USB_TRANSFER, 0 );
